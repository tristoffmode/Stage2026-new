...
import { View, Text, ScrollView, RefreshControl, Alert, TextInput, Platform, Dimensions, FlatList, ListRenderItemInfo } from 'react-native';
...
/**
 * Écran d'affichage des capteurs pour l'application.
 * 
 * @component
 * @returns {React.FC} Composant fonctionnel représentant l'écran d'affichage des capteurs.
 * 
 * @description
 * Ce composant affiche une liste de capteurs avec des options de tri et de filtrage par établissement.
 * Il permet également d'actualiser les données via un pull-to-refresh et d'interagir avec les capteurs
 * pour afficher des détails ou ajouter des notes.
 * 
 * @remarks
 * - Utilise `CapteurService` pour récupérer les données des capteurs et des sites.
 * - Gère les filtres de tri (température, humidité, batterie, activité) et les établissements.
 * - Inclut un système de gestion des clics sur les métriques et un modal pour ajouter des notes.
 * - Réinitialise le délai d'inactivité via `AuthService.resetTimeout`.
 */

const AffichageCapteurs: React.FC = () => {
	...
	const FILTERS_BAR_HEIGHT = 90;
	const listTopOffset = insets.top + SEARCH_BAR_HEIGHT + FILTERS_BAR_HEIGHT;
	const scrollMarginTop = isWebPlatform ? listTopOffset - 10 : listTopOffset;
	const flatListPaddingTop = isWebPlatform ? listTopOffset + 10 : listTopOffset;
	const numColumns = isWebVersion
		? (windowWidth > 1200
			? 4                // Grand PC: 4 columns
			: (windowWidth > 900
				? 3            // PC: 3 columns
				: 2))          // Tablette: 2 columns
		: 1; // 3 ou 4 colonnes en version web
	...
	const fetchData = async () => {
		...
		try {
			...
			console.log("Récupération des données actualisées des capteurs");
	...
	const ouvrirModalNotes = (index: number): void => {
		AuthService.resetTimeout(router);
		setIndexCapteurActuel(index);
		setModalNotesVisible(true);
	};

	const renderGridItem = ({ item, index }: ListRenderItemInfo<Capteur>) => {
		return (
			<View style={{
				flex: 1,
				margin: 8,
				...
				<CapteurCard
					...
					ouvrirModalNotes={ouvrirModalNotes}
					mode="affichage"
	...
	return (
		<View style={[styles.conteneur, { paddingTop: insets.top }]}>
			...
			<ModalAjouterNote
				...
				capteur={indexCapteurActuel !== null ? capteursFiltres[indexCapteurActuel] : null}
