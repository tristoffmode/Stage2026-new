...
const PlageHorairePicker: React.FC<PlageHorairePickerProps> = ({
    ...
    const sauvegarderEtFermer = () => {
        if (heureModifiee.debut > heureModifiee.fin) {
            const temp = new Date(heureModifiee.debut);
            heureModifiee.debut = new Date(heureModifiee.fin);
            heureModifiee.fin = temp;
        }
        
        setPlageHoraire(heureModifiee);
        setVisible(false);
    };
