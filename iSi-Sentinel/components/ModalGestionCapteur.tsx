...
const ModalGestionCapteur: React.FC<ModalGestionCapteurProps> = ({
	...
	const ouvrirSelecteurHeureDepuisModalJours = useCallback((jour: string, type: 'debut' | 'fin' = 'debut'): void => {
		// Close the days modal first so the time picker is not blocked behind it.
		setModalPlagesVisible(false);
		setTimeout(() => {
			ouvrirSelecteurHeure(jour, type);
		}, 180);
	}, [ouvrirSelecteurHeure]);
		...
		try {
			...
			if (notificationsActives) {
				const schedulesToUpdate = Object.entries(jours)
					...
					.map(([jour, plage]) => ({
						jour,
						debut: formatTimeHHMM(plage.debut),
						fin: formatTimeHHMM(plage.fin),
						actif: true
					}));

				console.log('Mise à jour des plages horaires:', schedulesToUpdate.length, 'plages');
				const schedulesSaved = await CalendrierService.updateAllSchedules(capteur.id, schedulesToUpdate);
				if (!schedulesSaved) {
					throw new Error('Echec de sauvegarde des plages horaires');
				}
			} else {
				console.log('Notifications désactivées, suppression de toutes les plages horaires');
				const schedulesCleared = await CalendrierService.updateAllSchedules(capteur.id, []);
				if (!schedulesCleared) {
					throw new Error('Echec de suppression des plages horaires');
				}
			}
			...
			<View style={styles.fondModal}>
				<View style={[
					...
					<ModalPlagesHoraires
						...
						jours={jours}
						setJours={setJours}
						ouvrirSelecteurHeure={ouvrirSelecteurHeureDepuisModalJours}
					/>
