...
import { addDays, format, subDays, parse, differenceInCalendarDays } from 'date-fns';
...
export const StatistiquesProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
	...
	useEffect(() => {
		const loadCapteurs = async () => {
			...
			try {
				...
				const allCapteurs = await CapteurService.getCapteurs();
				const capteursData = selectedSiteLabel
					? allCapteurs.filter((c: import('../services/capteurService').Capteur) => c.site_name === selectedSiteLabel)
					: allCapteurs;
	...
	const loadCapteurData = useCallback(async () => {
		...
		try {
			...
			if (result.error && !result.error.includes('Aucune donnée')) {
				...
				if (result.error && result.error.includes('Aucune donnée')) {
					...
					let emptyTableData: ProcessedData[] = [];

					if (useHourFormat) {
						const endHour = endDate.getHours();
						emptyLabels = Array.from({ length: endHour + 1 }, (_, i) => `${String(i).padStart(2, '0')}:00`);
						emptyTableData = emptyLabels.map(hour => ({
							Date_Time: hour,
							Temp_Moyenne: null,
							Temp_Minimum: null,
							Temp_Maximum: null,
					...
					setTemperatureData(emptyLabels.map(() => null));
					setTemperatureMin(emptyLabels.map(() => null));
					setTemperatureMax(emptyLabels.map(() => null));
					setHumidityData(emptyLabels.map(() => null));
					setBatteryData(emptyLabels.map(() => null));
	...
	useEffect(() => {
		if (selectedCapteur && selectedSite) {
			loadCapteurData();
		}
	}, [selectedCapteur, selectedSite, loadCapteurData]);
	...
	const onEndDateChange = (event: any, selectedDate?: Date) => {
		...
		if (selectedDate) {
			...
			// If selected day is today → use current time; otherwise use end of day
			const isToday =
