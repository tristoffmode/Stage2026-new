...
static async getCapteurs(): Promise<Capteur[]> {
	try {
		console.log('Requête API pour obtenir les capteurs');
		const timestamp = new Date().getTime();
		const response: AxiosResponse = await this.axiosInstance.get(`/api/capteurs?t=${timestamp}`);
...
static async getCapteurNotes(capteur_id: number): Promise<any[]> {
	try {
		const notes = await NoteService.getNotes(capteur_id);
		return notes;
	} catch (error: any) {
